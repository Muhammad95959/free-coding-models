 ...
 * @description WebGL Dithering shader background for dark mode landing page.
 */
"use client"
...
void main() {
    ...
    float noise = snoise(uv * 1.5 + vec2(uTime * 0.04, uTime * 0.02)) * 0.25;
    float diagonal = (uv.x + uv.y) * 0.5;
    float gradient = diagonal * 1.2 + noise;
    
    vec3 deepDark = uColor2;
    vec3 accentColor = uColor1;
    vec3 softAccent = mix(deepDark, accentColor, 0.25);
    vec3 midAccent = mix(deepDark, accentColor, 0.55);
    ...
    color = mix(color, color * 0.8, (1.0 - vignette) * 0.4);
...
export default function HeroGeometric({
    ...
    return (
        <div
            className={cn("relative w-full min-h-screen flex flex-col items-center overflow-hidden bg-bg", className)}
            style={{ containerType: "size" }}
        >
            <div className="absolute top-0 left-0 w-full h-full z-0 pointer-events-none opacity-60 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-accent/20 via-bg to-bg">
                <Canvas
                            ...
                            {title1 && (
                                <div className="overflow-hidden">
                                    <motion.h1
                                        ...
                                        className="text-[12cqi] md:text-[8cqi] lg:text-[6cqi] leading-[0.9] tracking-tighter text-fg-muted"
                                    >
                                        <span className="font-serif italic font-light text-fg">
                                            {title1}
                                        </span>
                                    </motion.h1>
                                </div>
                            ...
                            {title2 && (
                                <div className="overflow-hidden">
                                    <motion.h1
                                        ...
                                        className="text-[12cqi] md:text-[8cqi] lg:text-[6cqi] leading-[0.9] tracking-tighter font-bold text-fg"
                                    >
                                        {title2}
                                    </motion.h1>
                                </div>
                        ...
                        {description && (
                            <div className="max-w-[480px] text-center mb-8">
                                <motion.p
                                    ...
                                    className="text-lg md:text-[1.35rem] leading-relaxed text-fg-muted font-normal"
                                >
                                    {description}
                                </motion.p>
                            </div>
