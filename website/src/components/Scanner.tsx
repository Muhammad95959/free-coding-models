 ...
 * @description React Bits "Scanner" WebGL background (ogl): animated scan
 *   bands bent by a signal field, with optional CRT scanlines and grain.
 *   Used behind the sticky site header. Pauses off-screen / on hidden tab,
 *   renders a single static frame when the user prefers reduced motion,
 *   and degrades silently if the WebGL context cannot be created.
 *
 * @see https://reactbits.dev/tools/background-studio
 */
...
const hexToRgb = (hex: string): [number, number, number] => {
	...
	if (!match?.[1] || !match[2] || !match[3]) return [1, 1, 1];
	return [
...
const Scanner: React.FC<ScannerProps> = ({
	...
	useEffect(() => {
		...
		try {
			...
			// No WebGL2 available: degrade to a plain transparent background.
			setFailed(true);
			return;
		}
		...
		// Reduced motion: keep a static frame, never start the RAF loop.
		const reducedMotion =
